...
class SimulationConfigGenerator:
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Reserve 500 characters buffer
...
- The user base is Chinese, and the schedule must follow Beijing Time daily routines
- Almost no activity from midnight to 5 AM (activity multiplier 0.05)
...
- Activity decreases after 11 PM (activity multiplier 0.5)
- General pattern: low activity in early morning, gradually increasing in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only. You should adjust the specific time periods based on the nature of the event and participant characteristics
  - For example: Student groups may peak from 9-11 PM; media outlets are active all day; official institutions only during work hours
...
- peak_hours (int array): Peak hours, adjust based on the event's participant groups
...
- reasoning (string): Brief explanation of the configuration rationale"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. The time configuration must follow Chinese daily routines."
