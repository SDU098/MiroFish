...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        entities and relationships from it. No simulation-related prefix is
        added to avoid misleading the graph update.
        """
    ...
    def _describe_like_post(self) -> str:
        """Like a post - includes the original post content and author info"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post - includes the original post content and author info"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_quote_post(self) -> str:
        """Quote a post - includes the original post content, author info, and quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    to the Zep graph in real time. Activities are grouped by platform and sent
    to Zep in batches after accumulating BATCH_SIZE entries.
    ...
    # Send interval (seconds) to avoid overly rapid requests
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._failed_count = 0      # Total batches that failed to send
        self._skipped_count = 0     # Total activities filtered out (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full context information (e.g., original post content, user names, etc.).
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Send interval to avoid overly rapid requests
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_count": self._failed_count,          # Total batches that failed to send
            "skipped_count": self._skipped_count,        # Total activities filtered out (DO_NOTHING)
